...
} from "../../generated/FortuneRollV1/FortuneRollV1"
...
export function handleNewBet(event: NewBetEvent): void {
  ...
  options[1] = event.params.betOn.toI32() & ~(1 << 16)
...
export function handleNewBetResult(event: NewBetResultEvent): void {
  ...
  let referrerId: BigInt = _resolveGameResult(
    ...
    event.params.gameId,
    event.transaction.hash,
    getMultiplier(event.params.win, gameResult.betOn!,game.houseEdge.toString(),'fortuneroll', event.params.result.toI32()),
    event.params.result,
    event.params.win,
