...
export function handleNewBet(event: NewBetEvent): void {
    ...
    let options = new Array<i32>(1)

    options[0] = event.params.betOn.toI32()

    // log.info("LOG betON: {}", [event.params.betOn.toString()])
    _getOrCreateGameType(
        type,
        STARTING_HOUSE_EDGE
    )
...
export function handleNewBetResult(event: NewBetResultEvent): void {
    ...
    let referrerId: BigInt = _resolveGameResult(
        ...
        event.params.gameId,
        event.transaction.hash,
        getMultiplier(event.params.win, gameResult.betOn!, game.houseEdge.toString(), "rps", event.params.result.toI32()),
        event.params.result,
        event.params.win,
    ...
    log.info('resolveGame 2 : Player has the referralId {}', [referrerId.toString()])
    ...
    log.info('isAlreadyActive : {}', [isAlreadyActive.toString()])
