...
export function _resolveGameResult(
    ...
    if (referralProfile == null) {
        referralProfile = new ReferralProfile(player.toHexString())
        ...
        referralProfile.timestamp = timestamp
    }

    if (entity.referralId!.notEqual(BigInt.fromString("0"))) {

        let refCode = ReferralCode.load(entity.referralId!.toString())

        if (refCode != null) { // check if refCode exist, player can use a false refCode

            let referrerProfile = ReferralProfile.load(refCode.owner)! // get ReferralProfile of the owner to get timestamp

            if (referrerProfile.timestamp.lt(referralProfile.timestamp)) { // verify if player signed after referrer

                if (refCode.owner.toLowerCase() != player.toHexString().toLowerCase()) { // verify if not self referred
                    
                    if (refCode.code != referralProfile.referrerCode){
                        referralProfile.fiatVolume = ZERO_BD
                    }


                    referralProfile.referrer = refCode.owner
                    referralProfile.referrerCode = refCode.code
                    referralProfile.referredAt = timestamp
    ...
    if (referralProfile.referrerCode != null) { // update referrerID if the player has a referrerCode

        let referralCode = Code.load(referralProfile.referrerCode!)!

        referrerID = BigInt.fromString(referralCode.codeId.toString())
    }
    referralProfile.save()
    return referrerID
}
...
export function _getOrCreateReferralProfileAndCheckForReferrer(
    ...
    let hasNoReferrer : boolean = true
    let isFirstGame : boolean = false
    let referrrerAddress : string | null = null

    // let RefCheck = new ReferralCheck(true, false, null)

    if (entity.referrerCode != null){
        hasNoReferrer = false
    }

    if (entity.fiatVolume == ZERO_BD && entity.referrerCode != null){
        isFirstGame = true
        entity.fiatVolume = entity.fiatVolume!.plus(fiatAmount)
        entity.save()
    }
    ...
    if (entity.referrer != null){
        referrrerAddress = entity.referrer
    }

    let RefCheck = new ReferralCheck(hasNoReferrer,isFirstGame,referrrerAddress)
...
function updateBaseCommission(volume: BigDecimal, activeReferees: number, referrer: string): void {
    ...
    // let c = oracles.chainlink.fetchPriceUSD(tokenAddress);

    // // get token price in usd from YearnLens oracle
    // let y = oracles.yearnLens.fetchPriceUSD(tokenAddress);

    // // get token price in usd from Uniswap oracle
    // let u = oracles.uniswap.fetchPriceUSD(tokenAddress);

    // log.info('chainlink {},yearnlens {},uniswap {}',[c.toString(),y.toString(),u.toString()])
    let reqVolume = BigDecimal.fromString('50')

    if (volume.ge(reqVolume)) {
...
export function getMultiplier(isWin: boolean, betOn: i32[], houseEdge: string, type: string, result: number ): BigDecimal {
