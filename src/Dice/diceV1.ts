...
export function handleNewBetResult(event: NewBetResultEvent): void {
  ...
  let referrerId: BigInt = _resolveGameResult(
    ...
    event.params.gameId,
    event.transaction.hash,
    getMultiplier(event.params.win, gameResult.betOn!,game.houseEdge.toString(),"dice", event.params.result.toI32()),
    event.params.result,
    event.params.win,
